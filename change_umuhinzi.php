...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT id, amazina, ibanga FROM umuhinzi WHERE id = ?");
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        if ($stmt->execute()) {
            echo '<script>alert("Amazina n\'ijambo ry\'ibanga byahinduwe neza!"); window.location="umuhinzilogin.php";</script>';
        } else {
            echo '<script>alert("Habaye ikosa, ongera ugerageze."); window.location="change_umuhinzi.php";</script>';
        }
    } else {
        echo '<script>alert("Ntago wabonetse muri database."); window.location="change_umuhinzi.php";</script>';
    }
...
<body>
<p style="background-color: green; position: top;  position: absolute; top: 5px;height: 50px;width: 140px;margin-top: 2px;white-space: 2px;"> <a href="page2.php" style="text-decoration: none;">garuka inyuma</a></p>
    <form method="POST">
